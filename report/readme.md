...
2. Import the eye tracking recordings you want to analyze by doing the following:
    ...
    c. Select the eye tracker for which you want to find importable recordings.
    d. Assign recordings to sessions by dragging and dropping them in the GUI that pops up.
    e. If the imported recording does not include a camera calibration for the scene camera, set it now by indicating which XML file to use.
