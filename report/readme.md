...
6. If you want to use devkit or prototype eye trackers, additionally register them in gazeMapper using the `Registered custom eye trackers` setting. See the note about Generic eye trackers in the [glassesTools readme](https://github.com/dcnieho/glassestools?tab=readme-ov-file#eye-tracker-support). Information about the generic eye tracke is stored in the [`eye_trackers.json`](eye_trackers.json) file using the key `Generic.<eye tracker name>`, where `<eye tracker name>` is the exact name you used to register the generic eye tracker in gazeMapper.
