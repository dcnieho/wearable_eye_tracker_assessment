...
def run_validation(win: visual.Window, config: dict, refresh_rate: int, task_vars: dict):
    # prepare visual objects
...
def run_psa(win: visual.Window, config: dict, screen_config: list[float]):
    # prepare visual objects
    fixation_target  = ABCFixPoint(win,
                                   outer_diameter=config["target"]["outer_diameter"], inner_diameter=config["target"]["inner_diameter"],
                                   outer_color   =config["target"]["outer_color"]   , inner_color   =config["target"]["inner_color"],
    ...
    fiducials_array  = [visual.Circle(win, pos=pol2cart(config["fiducials"]["rho"][r], phi/180*np.pi), radius=config["fiducials"]["radius"], units=config["fiducials"]["units"], edges='circle', fillColor=config["fiducials"]["color"])
    ...
    try:
        ...
        for r in range(config["validation"]["n_repetitions"]):
            ...
            for m_id in config["validation"]["segment_marker"]["start_IDs"]:
                segmenter.draw(m_id)
            # run validation
            run_validation(win, config["validation"], config["screen"]["refresh_rate"], task_vars)
